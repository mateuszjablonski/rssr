...
if ($php_version < 8) {
    throw new ErrorException(
        "Invalid PHP version: expected >=8.0.0, got $php_version"
    );
}
...
$saved_article_urls = $database->query("SELECT url FROM articles")->fetchAll();
...
$site_description =
    "Najpopularniejsze źródło informacji na temat małego i dużego lotnictwa";
...
$main_view = $main_document->querySelector(
    "div.view-articles div.view-content"
);
...
foreach ($articles_missing as $article_url) {
    ...
    // extract title
    $title = $article->querySelector(".field--name-node-title h2")
        ?->textContent;
    $title = trim($title);
    $title = str_replace(
        "html5-dom-document-internal-entity1-quot-end",
        '"',
        $title
    );
    ...
    // extract date
    $date_time = $article->querySelector(
        "div.field--name-node-post-date div.field__item span.item"
    )?->textContent;
...
debug_log(
    "Articles inserted to database: $inserted_count of $not_present_count"
);
